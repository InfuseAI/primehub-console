...
class ScheduleDetailContainer extends React.Component<Props> {
  ...
  cancel = values => {
    ...
    if (isEqual(values, initialValue)) return this.back();
  ...
  back = () => {
    ...
    if (pathname) return history.push(`${pathname}${search}`);
  ...
  render() {
    ...
    const selectedGroup =
      this.state.selectedGroup || get(getPhSchedule, 'phSchedule.groupId');
