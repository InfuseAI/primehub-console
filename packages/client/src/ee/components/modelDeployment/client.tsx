            ...
            {form.getFieldDecorator('client', {
              rules: [
                { required: true, message: 'Please input your client name!' },
                {
                  ...
                  message: `Should match the pattern ^[a-z_][a-z0-9_-]*`,
                },
              ],
            })(
              <Input
        ...
        {clientAdded ? (
          <>
            <Form.Item label='Client Token'>
              <Input
                addonAfter={<a onClick={this.copyToken}>Copy</a>}
                value={clientAdded.plainTextToken}
                ref={this.refToken}
              />
            </Form.Item>
  ...
  render() {
    ...
    return (
      <div style={{ padding: '16px 36px' }}>
        <Row gutter={32}>
          <Col span={12}>{this.renderAddClient()}</Col>
          <Col span={12}>{this.renderClientList()}</Col>
        </Row>
      </div>
    );
  }
