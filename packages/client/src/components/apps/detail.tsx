...
export default class Detail extends React.Component<Props, State> {
  ...
  handleDelete = () => {
    ...
    confirm({
      ...
      content: (
        <span>
          Do you really want to uninstall "<b>{phApplication.displayName}</b>"?
        </span>
      ),
  ...
  stopped(): boolean {
    ...
    return (
      phApplication.status === PhAppStatus.Stopped ||
      phApplication.status === PhAppStatus.Stopping
    );
  }
  ...
  render() {
    ...
    const breadcrumbs = [
      ...
      },
      {
        ...
        tipsLink:
          'https://docs.primehub.io/docs/primehub-app#installed-app-detail',
      },
    ];
        ...
        <Card style={{ margin: '16px 16px 0' }}>
          <Left>
            <AppLogo
              ...
              src={phApplication.appIcon}
            />
          </Left>
          <h2 style={{ margin: '4px 0 0' }}>{phApplication.displayName}</h2>
          <h4 style={{ color: '#aaa' }}>
            {appTemplate.name}, app version: {phApplication.appVersion}
          </h4>
          <ClearBoth />
