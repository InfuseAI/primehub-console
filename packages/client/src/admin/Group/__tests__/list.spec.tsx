...
describe('Admin Portal - List Component', () => {
  ...
  it('Render rows properly', () => {
    ...
    const {container} = render(<List dataSource={groupsFakeData} columns={columns}/>);
    expect(container.querySelectorAll('tr[data-row-key]').length).toBe(groupsFakeData.length);
  });
  ...
  describe('List Component - Render loading', () => {
    ...
    it('Should render loading mask when provide loading prop equal true', () => {
      ...
      const {container} = render(<List dataSource={groupsFakeData} loading={true} columns={columns}/>);
...
describe('Admin Portal - Group List', () => {
  ...
  it('Render Group list with breadcrumb', () => {
    const {container} = render(<GroupList dataSource={groupsFakeData}/>, {wrapper});
    expect(screen.queryByText('Groups')).toBeInTheDocument();
  });

  it('Should enable delete when disableGroup is falsy or not present', () => {
    const {container} = render(<GroupList dataSource={groupsFakeData}/>, {wrapper});
    expect(container.querySelector('[data-testid="delete-button"]')).not.toHaveAttribute('disabled');
  });
  ...
  it('Should disable delete when disableGroup is True', () => {
    ...
    const {container} = render(<GroupList dataSource={groupsFakeData}/>, {wrapper});
    expect(container.querySelector('[data-testid="delete-button"]')).toHaveAttribute('disabled');
  });
