...
export default class ArrayBreadcrumb extends Component<Props> {
  ...
  componentDidMount() {
    ...
    const [keyName, index, imageName] = paths;
    ...
    client.query({
      ...
      const group = get(result.data, ['group', 'edges', index, 'node', imageName], []);
      const everyoneGroup = get(result.data, ['everyoneGroup', 'edges', '0', 'node', imageName], []);
