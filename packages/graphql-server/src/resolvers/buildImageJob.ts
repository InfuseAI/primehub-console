...
export const transform = (item: Item<ImageSpecJobSpec, ImageSpecJobStatus>, namespace: string, graphqlHost: string): BuildImageJob => {
  return {
    ...
    logEndpoint: `${graphqlHost}/logs/namespaces/${namespace}/jobs/${item.metadata.name}`,
...
// tslint:disable-next-line:max-line-length
const listQuery = async (client: CustomResource<ImageSpecJobSpec>, where: any, namespace: string, graphqlHost: string): Promise<BuildImageJob[]> => {
  if (where.id) {
    const imageSpecJob = await client.get(where.id);
    return [transform(imageSpecJob, namespace, graphqlHost)];
  }

  const imageSpecJobs = await client.list();
  const transformedimageSpecJobs = imageSpecJobs.map(job => transform(job, namespace, graphqlHost));
...
export const query = async (root, args, context: Context) => {
  ...
  // tslint:disable-next-line:max-line-length
  const imageSpecJobs = await listQuery(crdClient.imageSpecJobs, args && args.where, context.namespace, context.graphqlHost);
...
export const queryOne = async (root, args, context: Context) => {
  ...
  try {
    ...
    return transform(imageSpec, context.namespace, context.graphqlHost);
  } catch (e) {
    return null;
  }
};
